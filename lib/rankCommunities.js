...
function indexProperty(propertyKey, propertyValue, gMeanStdDev,
  ...
  async.each(propertyValue, function(communityInfo, cb) {
    if (!indexedCommunities[communityInfo.communityID]) {
      ...
      satisfyingPreference[propertyKey] =
          normalizeData.convertToPctOfAvg(gMeanStdDev.gmean[propertyKey],
      ...
      indexedCommunities[communityInfo.communityID]
          .satisfyingPreferences[propertyKey] =
              normalizeData.convertToPctOfAvg(gMeanStdDev.gmean[propertyKey],
                                              gMeanStdDev.gstddev[propertyKey],
                                              communityInfo[propertyKey]);
...
function getPctOfAvgFromPreferences(satisfyingPreferences, gMeanStdDev,
...
function addBadPreferencesCommunity(reqPreferences, satisfyingPreferences,
  ...
  async.forEachOf(satisfyingPreferences, function(preferenceVal, preferenceKey,
    ...
    if (!community.satisfyingPreferences[preferenceKey]) {
      ...
      if ((reqPreferences[preferenceKey].op === '<' && pctOfAvg < 1) ||
          (reqPreferences[preferenceKey].op === '>' && pctOfAvg > 1)) {
...
function addBadPreferences(reqPreferences, indexedCommunities, gMeanStdDev,
...
function indexSatisfyingCommunities(reqPreferences, satisfyingPreferences,
  ...
  async.forEachOf(satisfyingPreferences, function(satisfyingPreferenceVal,
    ...
    indexProperty(satisfyingPreferenceKey, satisfyingPreferenceVal, gMeanStdDev,
    ...
    if (err) {
      ...
      addBadPreferences(reqPreferences, indexedCommunities, gMeanStdDev,
