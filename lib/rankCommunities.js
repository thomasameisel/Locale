...
function indexProperty(propertyKey,propertyValue,callback) {
    async.each(propertyValue,function(communityInfo,cb) {
        if (!indexedCommunities[communityInfo.communityID]) {
            ...
            indexedCommunities[communityInfo.communityID].pctOfAvgArr.push(communityInfo[propertyKey]);
            indexedCommunities[communityInfo.communityID].correspondingTypeArr.push(propertyKey);
        }
        cb();
    },function(err) {
        if(err) {
...
/*
keys in satisfyingProperties must have the same name as the keys in the communityInfo object
ex. satisfyingProperties={nightlifePctOfAvg:[{communityID:1,nightlifePctOfAvg:2,crimePctOfAvg:1}],
...
function indexSatisfyingCommunities(satisfyingProperties,callback) {
    ...
    async.each(Object.keys(satisfyingProperties),function(satisfyingProperty,cb) {
        indexProperty(satisfyingProperty,satisfyingProperties[satisfyingProperty],function(err) {
            if (err) {
                cb(err);
            } else {
                cb();
