    ...
    function(cb) {
      tribuneData.communitiesCrimePctOfAvg(communitiesInfo,
        ...
        if (err) {
          ...
          for (var data in violentResult) {
            if (violentResult.hasOwnProperty(data)) {
              nonViolentResult[data] = (1 / nonViolentResult[data]);
              violentResult[data] = (1 / violentResult[data]);
            }
          }
      ...
      getAllExternalData(communitiesInfo, function(err, result) {
        if (err) {
          ...
          var dataByCommunity = associateDataByCommunity(communityIDs,
                                                          allExternalData);
          return cb(null, dataByCommunity);
        }
      });
    },
    insertAllDataIntoDb
