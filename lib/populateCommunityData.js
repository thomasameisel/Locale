...
var streetEasyData = require('./streeteasy');
var normalizeData = require('./normalizeData');
var noiseData = require('./noiseData');

function getAllExternalDataChicago(communitiesInfo, callback) {
  var allExternalData = {};
  async.parallel([
    // function(cb) {
...
function getAllExternalDataNewYork(communitiesInfo, callback) {
  var allExternalData = {};
  async.parallel([
    function(cb) {
      nightlife.communitiesNightlifePctOfAvg(communitiesInfo,
    ...
    function(cb) {
      streetEasyData.communitiesPricePctOfAvg(communitiesInfo,
          function(err, result) {
        if (err) {
          return cb(err);
        } else {
...
function populateCommunityData(city, callback) {
  async.waterfall([
    ...
    function(communitiesInfo, cb) {
      switch (city) {
        case 'CHICAGO': {
          getAllExternalDataChicago(communitiesInfo, function(err, result) {
            if (err) {
              return cb(err);
            } else {
        ...
        case 'NEWYORK': {
          getAllExternalDataNewYork(communitiesInfo, function(err, result) {
            if (err) {
              return cb(err);
            } else {
              var allExternalData = normalizeData.normalizeData(result);
...
populateCommunityData('NEWYORK', function() {
  var endTime = new Date().getTime();
  console.log('Time elapsed:', (endTime - startTime) / 1000, 's');
});
