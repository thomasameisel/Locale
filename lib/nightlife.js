...
function getVenuesPerLandArea(communityInfo,callback) {
    var parameter = {ll:communityInfo.latLng,sw:communityInfo.sw,ne:communityInfo.ne,limit:'50',intent:'browse',
            categoryId:nightlifeCategoryID};
    foursquare.searchVenues(parameter, function(err,response) {
        if(err) {
            callback(err);
        } else {
            callback(null, (response.response.venues.length / communityInfo.landArea));
        }
    });
}

function communitiesNightlifePctOfAvg(communitiesInfo,callback) {
    communitiesPctOfAvg.communitiesPctOfAvg(communitiesInfo,getVenuesPerLandArea,function(err,result) {
        if(err) {
            callback(err);
        } else {
