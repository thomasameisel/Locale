...
function getAveragePriceFromArr(result, callback) {
    parseString(result, function (err, result) {
        if (err) {
            ...
            var listings = result.TruliaWebServices.response[0].TruliaStats[0].listingStats[0].listingStat;
            var oneBedroomSum = 0;
            if (listings) {
                for (var i = 0; i < listings.length; ++i) {
                    oneBedroomSum += parseInt(listings[i].listingPrice[0].subcategory[1].medianListingPrice[0]);
                }
...
function communitiesPricePctOfAvg(communitiesInfo, callback) {
    communitiesPctOfAvg.communitiesPctOfAvg(communitiesInfo, getCommunityPrice, function(err, result) {
        if (err) {
            callback(err);
        } else {
