...
function assignStreetEasyID(communities, callback) {
  async.forEachOfSeries(communities, function(value, key, cb) {
    streetEasy.findID(key, function(err, result) {
      if (err) {
        console.log(err, key, 'could not find streetEasyID');
      } else {
        communities[key].streetEasyID = result;
      }
      db.insertCommunityArea('NEWYORK', key, communities[key]);
...
console.log(nycMap.length);
var communities = [];
async.forEachSeries(nycMap, function(community, cb) {
  db.isNull(community.properties.neighborhood,
      function(err, result) {
    if (err) {
      cb(err);
    } else if (result) {
  ...
  if (err) {
    ...
    communities = communities.filter(function(item, pos, ary) {
      return !pos ||
          item.properties.neighborhood != ary[pos - 1].properties.neighborhood;
    });
    addAllPropertiesNYC(communities, function(err, result) {
      if (err) {
        console.log(err);
      } else {
