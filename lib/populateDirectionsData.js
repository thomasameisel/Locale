...
function populateDirections(city, boundaries, callback) {
  var boundariesArr = createBoundariesArr(boundaries);
  // var directionsByLatLng = {};
  var i = 0;
  async.eachSeries(boundariesArr, function(latLng, cb) {
    ...
    directionsCommunities.getTimeToCommunities(city, drivingPreferences,
      ...
      ++i;
      if (err) {
        console.error(err + ': ' + i + '/' + boundariesArr.length);
      } else {
        console.log('Successfully added ' + drivingPreferences.destination +
                    ': ' + i + '/' + boundariesArr.length);
        db.insertDirectionsData(city, drivingPreferences.destination, result);
      }
      return cb();
    });
  }, function(err) {
