...
function getTimeToCommunities(city, drivingPreferences, callback) {
  db.getAllCommunitiesInfo(city, function(err, communitiesInfo) {
    if (err) {
      return callback(err);
    } else {
...
function getClosestDbLatLng(city, latLng, callback) {
  db.getClosestLatLng(city, latLng, function(err, result) {
    if (err) {
      return callback(err);
    } else {
