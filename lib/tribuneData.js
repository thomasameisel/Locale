...
function getCrowdedPct(communityInfo, cb) {
  getAllCommunityData(communityInfo.communityID, function(err, response) {
    if (err) {
      console.log('error crowded', communityInfo.communityID);
      return cb(err);
    } else {
      console.log('finished crowded', communityInfo.communityID);
...
function communitiesCrimePctOfAvg(communitiesInfo, cb) {
  ...
  async.each(communitiesInfo, function(communityInfo, nextItr) {
    getCrimeCount(communityInfo, function(err, result) {
      if (err || !result) {
        console.log('error crime', communityInfo.communityID);
        return nextItr(err);
      } else {
        console.log('finished crime', communityInfo.communityID);
