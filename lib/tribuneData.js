...
function communitiesCrowdedPctOfAvg(communitiesInfo, cb) {
  rankCommunities.communitiesPctOfAvg(communitiesInfo, getCrowdedPct,
                                      function(err, result) {
    if (err) {
      cb(err);
    } else {
  ...
  makeRequest(SUMMARY_URL, params, function(err, response) {
    if (err) {
      ...
      dailyCrimeObjects.forEach(function(curObject) {
        ...
        numCrimes += curObject.total;
      });
      cb(null, (numCrimes / communityInfo.landArea));
    }
  });
}

function communitiesCrimePctOfAvg(communitiesInfo, cb) {
  rankCommunities.communitiesPctOfAvg(communitiesInfo, getCrimeCount,
                                      function(err, result) {
    if (err) {
      cb(err);
    } else {
